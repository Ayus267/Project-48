...
function draw(){
  ...
  fill("yellow");
  stroke("green");
  text("SCORE: "+score,width/2-300,60);
  ...
  if(gameState == Play) {
   ...
   textFont("Verdana");
   fill("red");
   text("LIVES: "+lives,width/2-10,60);
 ...
 if((bulletGroup.isTouching(asteroidGroup,bullet_Hit)))
 {
  score=score+1;
  blast_sound.play();
 }
...
function createAsteroid(){
  if(frameCount%40==0) {
  asteroid = createSprite(800,random(1,height + 100),40,40); 
